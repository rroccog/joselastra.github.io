...
summary(ctg)
```
.pull-left[![:scale 45%](las_cloud1.png)]
...
```{r eval=F}
#CHM
opt_output_files(normalize_ctg) <- '02_LiDAR/02_muestraOutput/02d_canopy/canopy_v1_{XLEFT}_{YBOTTOM}'
chm<- grid_canopy(normalize_ctg, res=1,
                  pitfree(thresholds = c(0,2,5,10,15,20), 
                          max_edge = c(0, 1.5)))
```

.center[![:scale 32.5%](chm2.png)
]
.center[.footnote[CHM with kernel filter [3x3]]]
---
```{r}
...
.footnote[Creado con [**xaringan**](https://github.com/yihui/xaringan)]
